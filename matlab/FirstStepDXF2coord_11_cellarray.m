...
%%%%%%% Below This Line is coded by Junchao Wang (jwang071@ucr.edu) %%%%%%% 
new_filename = regexp(filename, '.dxf', 'split');
new_filename = new_filename{1,1};
total = numel(polylines);
closed_polylines = 0;
open_polylines = 0;
for i = 1: total
    this_polyline = polylines{i, 1};
    if this_polyline(1, :) == this_polyline(end, :)
        closed_polylines = closed_polylines + 1;
    else
        open_polylines = open_polylines + 1;
    end
end
if closed_polylines == 0
    display('Looks like you do not have any closed polylins. Please manually closed it in variable {polylines} or closed it in autoCAD and try again! ')
    display('Please check this website for more information')
else
    save(strcat('./converted_geometry./', new_filename, '.mat'), 'polylines')
    display('No err. Success! Move to Step two!')
end
